...
def extract_project_info_from_sln(slnFile: str) -> list:
	...
	# Extract project lines using regex pattern - this handles GUIDs with curly braces
	...
	for project_type_guid, name, path_relative, project_guid in project_matches:
		# Replace Windows backslashes with forward slashes first to handle mixed path separators
		if '\\' in path_relative:
		...
		if path_relative.endswith(".csproj"):
			projects.append({
				'name': name,
				'path': path_relative,
				'guid': "{" + project_guid + "}",  # Include curly braces in the GUID
				'folder': folder_name,
				'is_csproj': True
			})
		else:
...
def rename_project_files(projects: list, project_mapping: dict, slnFile: str) -> None:
	"""
		...
		if project['is_csproj']:
			...
			if mapping:
				...
				if mapping.get('ignored', False):
					print(f"[*] INFO: Skipping file rename for ignored project: {project['name']}")
					continue
					
				# Get the old and new file paths
				old_project_path = os.path.join(os.path.dirname(slnFile), project['path'])
				...
				if os.path.exists(old_project_path):
					...
					new_project_path = os.path.join(os.path.dirname(old_project_path), new_file_name)
					new_project_path = normalize_path_for_os(new_project_path)
					
					try:
						print(f"[*] INFO: Renaming project file {old_project_path} to {new_project_path}")
						rename(old_project_path, new_project_path)
					except Exception as e:
						print(f"[!] WARNING: Could not rename project file {old_project_path}: {str(e)}")
				else:
					print(f"[!] WARNING: Project file not found for renaming: {old_project_path}")
					# Try an alternative approach with raw path for debugging
					raw_project_path = os.path.join(os.path.dirname(slnFile), project['path'].replace('\\', '/'))
					if os.path.exists(raw_project_path):
						# Get the new project file name
						old_file_name = os.path.basename(raw_project_path)
						new_file_name = mapping['new_name'] + ".csproj"
						new_project_path = os.path.join(os.path.dirname(raw_project_path), new_file_name)
						new_project_path = normalize_path_for_os(new_project_path)
						
						try:
							print(f"[*] INFO: Renaming project file {raw_project_path} to {new_project_path}")
							rename(raw_project_path, new_project_path)
						except Exception as e:
							print(f"[!] WARNING: Could not rename project file {raw_project_path}: {str(e)}")
			else:
				print(f"[!] WARNING: No mapping found for project: {project['name']}")
...
def stringObfuscate(theFile: str, project_mapping: dict, theObfMethod: str) -> None:
	"""
	...
	if file_project_name and project_mapping.get(file_project_name, {}).get('ignored', False):
		print(f"[*] INFO: Skipping string obfuscation for file in ignored project: {theFile}")
		return
	...
	for line in theLines:
		...
		if len(stringsInLine) > 0:
			...
			for old_name, mapping in project_mapping.items():
				strippedLine = strippedLine.replace(old_name, mapping['new_name'])
				...
				if canProceedWithObfuscation(line, theItem):
					...
					if theObfMethod == "rot13":
						...
						if "\\r" in strippedLine or "\\n" in strippedLine or "\\t" in strippedLine or "\"" in strippedLine or "\'" in strippedLine:
							...
							strippedLine = strippedLine.replace("\\e", "\\\\e").replace("\\g", "\\\\g").replace("\\\\\\e", "\\\\e").replace("\\\\\\g", "\\\\g")
							strippedLine = strippedLine.replace("++====THISGETSREPLACED====++", "\\" + "\"")  # remove placeholder strings
		...
		# if class has any project name in it, check if it matches the file name first
		elif "class " in line:
			modified_line = line
			
			# NEW CODE: Check if the line has a class definition matching the filename
			...
			if class_name_match:
				...
				if class_name == file_basename:
					print(f"[*] INFO: Preserving class name {class_name} in {theFile} as it matches the file name")
					fInCopy.write(line)
					continue
			...
			for old_name, mapping in project_mapping.items():
				if old_name in line:
					modified_line = modified_line.replace(old_name, mapping['new_name'])
		...
		# replace constructor name if it has project name in it
			...
			# NEW CODE: Check if the line has a constructor or method matching the file name
			constructor_match = search(r'(public|private)\s+([a-zA-Z0-9_]+)', line)
			if constructor_match:
				...
				if constructor_name == file_basename:
					print(f"[*] INFO: Preserving constructor/method {constructor_name} in {theFile} as it matches the file name")
					fInCopy.write(line)
					continue
			...
			if file.endswith(".dll"):
				...
				for old_name, mapping in project_mapping.items():
					...
					if old_name + ".dll" == file:
						...
						try:
							print(f"[*] INFO: Renaming DLL file {old_file_path} to {new_file_path}")
							rename(old_file_path, new_file_path)
						except Exception as e:
							print(f"[!] WARNING: Could not rename DLL file {old_file_path}: {str(e)}")
	
	chdir(orig_cwd)


def rename_project_supporting_files(theDirectory: str, project_mapping: dict) -> None:
	"""
			...
			if file.endswith(".snk"):
				...
				for old_name, mapping in project_mapping.items():
					...
					if old_name + ".snk" == file:
						...
						try:
							print(f"[*] INFO: Renaming SNK file {old_file_path} to {new_file_path}")
							rename(old_file_path, new_file_path)
						except Exception as e:
							print(f"[!] WARNING: Could not rename SNK file {old_file_path}: {str(e)}")
...
def apply_cloak(directory, name, obf_method=None, ignore_list=None, output_file=None):
	"""
	...
	if obf_method and obf_method not in ["base64", "rot13", "reverse"]:
		raise ValueError("Unsupported obfuscation method. Use 'base64', 'rot13', 'reverse', or None")
...
def main(theObfMethod: str, theDirectory: str, theName: str, ignore_list: list = None, output_file: str = None) -> None:
	"""
	...
	if csprojFile and not slnFile:
		...
		project_mapping = {
			project_name: {
				...
				'new_name': theName,
				'new_guid': str(uuid4()),
				'old_guid': str(uuid4()),  # We'll extract the real GUID from the project file
		...
		update_csproj_file(csprojFile, project_mapping[project_name], project_mapping)
		update_assembly_info(csprojFile, project_mapping[project_name])
		
		# Rename the project file
		rename_project_files([{'name': project_name, 'path': csprojFile, 'is_csproj': True}], project_mapping, csprojFile)
		...
		if theObfMethod != "":
			...
			for r, d, f in walk(theDirectory):
				for file in f:
					if file.endswith(".cs") and "AssemblyInfo.cs" not in file and not r.endswith(os.path.join("obj", "Debug")) and not r.endswith(os.path.join("obj", "Release")):
						stringObfuscate(os.path.join(r, file), project_mapping, theObfMethod)
	...
	for project in projects:
		if project['is_csproj']:
			...
			if ignore_list and project['name'] in ignore_list:
				is_ignored = True
				print(f"[*] INFO: Maintaining ignored status for project: {project['name']}")
					...
					if ignore_list:
						for ignored_project in ignore_list:
							if ignored_project in r:
								print(f"[*] INFO: Skipping string obfuscation for file in ignored project: {os.path.join(r, file)}")
								should_skip = True
								break
	...
	if ignore_list and len(ignore_list) > 0:
		...
		for r, d, f in walk(theDirectory):
			...
			if is_ignored_project:
				for file in f:
					if file.endswith(".cs") and "AssemblyInfo.cs" not in file and not r.endswith(os.path.join("obj", "Debug")) and not r.endswith(os.path.join("obj", "Release")):
						update_code_references(os.path.join(r, file), project_mapping)
...
def update_code_references(theFile: str, project_mapping: dict) -> None:
	...
	for old_name, mapping in project_mapping.items():
		...
		if new_content != file_content:
			...
			modified = True
			print(f"[*] INFO: Updated namespace references from '{old_name}' to '{mapping['new_name']}' in {theFile}")
		...
		if new_content != file_content:
			file_content = new_content
			modified = True
			print(f"[*] INFO: Updated fully qualified type references from '{old_name}' to '{mapping['new_name']}' in {theFile}")
...
def replaceGUIDAndToolName(theDirectory: str, theName: str, ignore_list: list = None) -> None:
	"""
	...
	if ignore_list and main_project['name'] in ignore_list:
		print(f"[!] WARNING: Main project '{main_project['name']}' is in the ignore list. This might cause unexpected behavior.")
		print(f"[!] WARNING: The main project will still be renamed to '{theName}' as specified.")
	...
	project_mapping[main_project['name']] = {
		...
		'new_folder': main_project['folder'],  # Keep the original folder name for the main project
		'is_main': True,
	...
	for i in range(1, len(projects)):
		...
		if project['is_csproj']:
			...
			if ignore_list and project['name'] in ignore_list:
				...
				# Store the original to new name mapping (same name for ignored projects)
				...
				project_mapping[project['name']] = {
					...
					'old_guid': project['guid'],
					'old_folder': project['folder'],
					'new_folder': project['folder'],  # Keep the original folder name
					'is_main': False,
					'ignored': False
				}
				# Store the original to new name mapping
	...
	for old_name, mapping in project_mapping.items():
		if mapping.get('ignored', False):
			...
			print(f"  - '{old_name}' → '{mapping['new_name']}' (GUID: {mapping['old_guid']} → {mapping['new_guid']})")
	...
	for project in projects:
		if project['is_csproj']:
			...
			print(f"[*] DEBUG: Solution file directory: {os.path.dirname(slnFile)}")
			
			# Use os.path.join for cross-platform path handling
			project_path = os.path.join(os.path.dirname(slnFile), project['path'])
			...
			if os.path.exists(project_path):
				...
				# Try an alternative approach with raw path for debugging
				print(f"[!] DEBUG: Checking raw project path from solution: {project['path']}")
				raw_project_path = os.path.join(os.path.dirname(slnFile), project['path'].replace('\\', '/'))
				if os.path.exists(raw_project_path):
					print(f"[+] Found project using alternative path: {raw_project_path}")
					mapping = project_mapping.get(project['name'])
					if mapping:
						print(f"[*] DEBUG: Project mapping: {mapping}")
						update_csproj_file(raw_project_path, mapping, project_mapping)
...
def update_solution_file(slnFile: str, project_mapping: dict) -> None:
	...
	for old_name, mapping in project_mapping.items():
		# Skip ignored projects
		if mapping.get('ignored', False):
			print(f"[*] INFO: Skipping solution project declaration updates for ignored project: {old_name}")
			continue
			
		# Strip curly braces for pattern matching
		old_guid_no_braces = mapping["old_guid"].replace("{", "").replace("}", "")
		new_guid_no_braces = mapping["new_guid"].replace("{", "").replace("}", "")
		
		# Find project declarations with this format:
		# Project("{PROJECT_TYPE_GUID}") = "ProjectName", "Path\ProjectName.csproj", "{PROJECT_GUID}"
		pattern = r'Project\("\{([^}]+)\}"\)\s+=\s+"' + escape(old_name) + r'",\s+"([^"]+)",\s+"\{' + escape(old_guid_no_braces) + r'\}"'
		
		def replacement_func(match):
			project_type_guid = match.group(1)  # The project type GUID
			path = match.group(2)  # The path to the .csproj file
			
			# Update the path to use the new project name but keep the same folder structure
			if '\\' in path:
				...
				if path.endswith(f"{old_name}.csproj"):
					...
					new_path = path.replace(f"{old_name}.csproj", f"{mapping['new_name']}.csproj")
				else:
					# Path may have folders that match the project name - we need to be careful
					...
					if path_parts[-1].endswith(".csproj"):
						...
						new_path = path[:last_index] + mapping['new_name'] + path[last_index + len(old_name):]
			else:
			...
			# Format the new project declaration with the updated name, path, and GUID
		...
		# Replace GUID references elsewhere in the file (like in ProjectDependencies)
		old_guid_formatted = "{" + old_guid_no_braces + "}"
		new_guid_formatted = "{" + new_guid_no_braces + "}"
		sln_content = sln_content.replace(old_guid_formatted, new_guid_formatted)
	
	# Make sure to update references to non-ignored projects within the GlobalSection sections
	global_sections = findall(r'GlobalSection\([^)]+\) = (\w+).*?EndGlobalSection', sln_content, re.DOTALL)
	for section in global_sections:
		...
		for old_name, mapping in project_mapping.items():
			if not mapping.get('ignored', False):
				...
				section = section.replace(old_guid_formatted, new_guid_formatted)
...
def update_csproj_file(csprojFile: str, mapping: dict, project_mapping: dict) -> None:
	"""
	...
	if is_ignored:
		print(f"[*] INFO: Processing references in ignored project: {mapping.get('old_name', 'Unknown')}")
	else:
	...
	if 'old_name' not in mapping:
		...
		if 'old_name' not in mapping:
			...
			if file_name.endswith('.csproj'):
				...
				print(f"[*] INFO: Using file name '{possible_name}' as old_name for project")
	...
	if not is_ignored:
		...
		csproj_content = csproj_content.replace(mapping["old_guid"], mapping["new_guid"])
		
		# Update AssemblyName element to match the new project name
		csproj_content = sub(
			r'<AssemblyName>' + escape(mapping["old_name"]) + r'</AssemblyName>',
			r'<AssemblyName>' + mapping["new_name"] + r'</AssemblyName>',
			csproj_content
		)
		
		# Update RootNamespace element to match the new project name
		csproj_content = sub(
			r'<RootNamespace>' + escape(mapping["old_name"]) + r'</RootNamespace>',
			r'<RootNamespace>' + mapping["new_name"] + r'</RootNamespace>',
			csproj_content
		)
		
		# Update signing key files (.snk)
		csproj_content = sub(
			r'<AssemblyOriginatorKeyFile>' + escape(mapping["old_name"]) + r'\.snk</AssemblyOriginatorKeyFile>',
			r'<AssemblyOriginatorKeyFile>' + mapping["new_name"] + r'.snk</AssemblyOriginatorKeyFile>',
			csproj_content
		)
	
	# Extract and preserve ItemGroup/Compile sections
	compile_items = []
	for match in findall(r'<ItemGroup>\s*(?:<Compile[^>]*>\s*)*</ItemGroup>', csproj_content):
		if '<Compile ' in match:
			compile_items.append(match)
	
	# Extract and temporarily remove OutputPath elements to completely protect them from changes
	output_paths = {}
	...
	csproj_content = sub(r'<OutputPath>[^<]+</OutputPath>', replace_output_path, csproj_content)
	
	# Extract and temporarily remove HintPath elements to protect folder references
	hint_paths = {}
	def replace_hint_path(match):
		...
		dll_name_match = search(r'([^\\/<>]+)\.dll</HintPath>', hint_path_content)
		
		if dll_name_match:
			dll_name = dll_name_match.group(1)
			# If this DLL name matches a project name that's being renamed, update just the DLL name
			for old_proj_name, proj_mapping in project_mapping.items():
				if dll_name == old_proj_name and not proj_mapping.get('ignored', False):
	...
	csproj_content = sub(r'<HintPath>[^<]+</HintPath>', replace_hint_path, csproj_content)
	
	# Replace references to other renamed projects for ALL projects (including ignored ones)
	# This is the key change - we want to update references even in ignored projects
	for old_name, other_mapping in project_mapping.items():
		...
		csproj_content = csproj_content.replace(other_mapping["old_guid"], other_mapping["new_guid"])
		
		# Handle specific DLL references in Reference Include and EmbeddedResource elements
		# Match References like: <Reference Include="ProjectName"> or <Reference Include="ProjectName.Something">
		csproj_content = sub(
		...
		# Match DLL files in EmbeddedResource like: <EmbeddedResource Include="path\ProjectName.dll" />
		csproj_content = sub(
			r'<EmbeddedResource Include="([^<]*)' + escape(old_name) + r'\.dll"', 
			r'<EmbeddedResource Include="\1' + other_mapping["new_name"] + r'.dll"', 
			csproj_content
		)
		
		# Use a targeted approach for project name replacement, excluding Compile Include sections
		# First, split the content into sections around ItemGroup containing Compile elements
		parts = []
		current_pos = 0
		...
		for item in compile_items:
			...
			if start_pos > current_pos:
				...
				section = sub(r'(?i)' + escape(old_name), other_mapping["new_name"], section)
	...
	if not is_ignored:
		csproj_content = csproj_content.replace("<DebugType>pdbonly</DebugType>", "<DebugType>none</DebugType>")
		csproj_content = csproj_content.replace("<DebugType>full</DebugType>", "<DebugType>none</DebugType>")
...
def update_assembly_info(projectPath: str, mapping: dict) -> None:
	...
	if mapping.get('ignored', False):
		print(f"[*] INFO: Skipping assembly info updates for ignored project: {mapping.get('old_name', 'Unknown')}")
		return
	...
	if 'old_name' not in mapping:
		...
		if file_name.endswith('.csproj'):
			possible_name = file_name[:-7]  # Remove .csproj extension
			mapping['old_name'] = possible_name
			print(f"[*] INFO: Using file name '{possible_name}' as old_name for assembly info")
	...
	if not assemblyInfoFile:
		print(f"[!] WARNING: AssemblyInfo.cs not found for project: {projectPath}")
		# Try an alternative approach with normalized paths
		print(f"[!] DEBUG: Searching for AssemblyInfo.cs with normalized paths in: {project_dir}")
		project_dir_normalized = os.path.normpath(project_dir)
		for r, d, f in walk(project_dir_normalized):
			for file in f:
				if "AssemblyInfo.cs" in file:
					assemblyInfoFile = os.path.join(r, file)
					print(f"[+] Found AssemblyInfo.cs using normalized path: {assemblyInfoFile}")
					break
			if assemblyInfoFile:
				break
	...
	assembly_content = sub(r'(?i)' + escape(mapping["old_name"]), mapping["new_name"], assembly_content)
	
	# Make sure GUID is properly formatted
	old_guid_formatted = mapping["old_guid"].replace("{", "").replace("}", "").lower()
	new_guid_formatted = mapping["new_guid"].replace("{", "").replace("}", "").lower()
	assembly_content = assembly_content.replace(old_guid_formatted, new_guid_formatted)
...
if __name__ == '__main__':
	try:
		parser = OptionParser(formatter=TitledHelpFormatter(), usage=globals()['__doc__'], version='0.6')
		...
		parser.add_option('-i', '--ignore', dest='ignore', help='comma-separated list of projects to ignore (e.g., "CommonDependencies,OtherProject")')
		parser.add_option('-o', '--output', dest='output', help='output CSV file for project mapping (e.g., "mapping.csv")')
		...
		if options.ignore:
			...
			print(f"[*] INFO: The following projects will be ignored: {', '.join(ignore_list)}")
